...
if(!empty($result))
{
  ...
  $embed_code = "<script src=\"http://koulifang.com/user/".$user_id."/".$story_embed.".js\"></script>";
	...
	if(count($tweibo_id_array) > 0)
	{
	  ...
	  foreach( $info as $item )
	  {
		...
		if(isset($item['source'])){
			...
			$tweiboContent .= "<div class='story_wrapper'><div class='quote_sign'>“</div><div class='content_wrapper'><span class='weibo_text_drop'>".$item['text']."</span></div>";
			if(isset($item['image'])){
