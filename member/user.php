	...
	if($login_status)
	{
	  ...
	  $content = "<div id='boxes' class='p_relative'>
					<div id='weibo_dialog' class='window".$extra_class."'>
					  ...
					  <div class='pub_imply_sina'><span>发布到新浪微博需要绑定新浪微博帐号</span><a href='/accounts/source.php'>现在去绑定</a></div>
					  <div class='pub_imply_tencent'><span>发布到腾讯微博需要绑定腾讯微博帐号</span><a href='/accounts/source.php'>现在去绑定</a></div>
				    </div>
				  </div>";
	}
	else
	...
	else
	{
	  if(0 == strcmp($story_status, 'Published'))
	  {
		...
		else
		{
		  if($w_array_length>0)
		  {
		    $content.="<div class='sina_user'>
					   <span>发布到新浪微博需要绑定新浪微博帐号</span><a href='/accounts/source.php'>现在去绑定</a>
					</div>";
		  } 
		}
		...
		else
		{
		  ...
		  {
		    $content.="<div class='tencent_user clear'>
					   <span>广播到腾讯微博需要绑定腾讯微博帐号</span><a href='/accounts/source.php'>现在去绑定</a>
					</div>";
		  }
		}
...
else if(isset($_GET['user_id']) && !isset($_GET['post_id']))
{
  ...
  $story_content = "<div id='userstory_container' class='inner'>
					  <div class='userinfo_wrapper'>
						<div class='avatar'><a href='/member/user.php?user_id=".$user_id."'><img width='80px' height='80px' src='".$user_avatar_img."' alt='".$username."' /></a></div>
						<div class='misc_wrapper'>
						  <div class='user_name'><a href='/member/user.php?user_id=".$user_id."'><span>".$username."</span></a></div>
