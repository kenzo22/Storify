...
foreach( $weibo as $item )
{
  ...
  style = 'display:block;'><span class='weibo_from'>".$item['user']['screen_name']."</span></a><span class='weibo_text'>".$item['text']."</span><div>";
    ...
    if (isset($item['retweeted_status'])){
        ...
        if(isset($item['retweeted_status']['thumbnail_pic'])){
            $weiboContent .= "</span><img src='".$item['retweeted_status']['thumbnail_pic']."' >";
        }
    }
    if (isset($item['thumbnail_pic']))
        $weiboContent .= "</span><img src='".$item['thumbnail_pic']."' >";
